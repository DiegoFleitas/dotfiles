...
@test "dot_nvmrc major matches node pin in dot_mise.toml" {
  local node_pin
  node_pin="$(grep -E '^node\s*=' "${REPO_ROOT}/dot_mise.toml" | sed -E 's/^node[[:space:]]*=[[:space:]]*"([^"]+)".*/\1/')"
  local nvmrc
...
@test "README mentions Node Python PHP versions from dot_mise.toml" {
  ...
  node_pin="$(grep -E '^node\s*=' "${dot_mise}" | sed -E 's/^node[[:space:]]*=[[:space:]]*"([^"]+)".*/\1/')"
  python_pin="$(grep -E '^python\s*=' "${dot_mise}" | sed -E 's/^python[[:space:]]*=[[:space:]]*"([^"]+)".*/\1/')"
  php_pin="$(grep -E '^php\s*=' "${dot_mise}" | sed -E 's/^php[[:space:]]*=[[:space:]]*"([^"]+)".*/\1/')"
...
@test "dot_mise.toml missing required tool line is detectable" {
  ...
  grep -v '^php\s*=' "${DOT_MISE_BACKUP}" > "${dot_mise}"
  run grep -E '^php\s*=' "${dot_mise}"
  [ "$status" -ne 0 ]
